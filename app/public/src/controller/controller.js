class Controller{
    ...
    initEvents(){
        ...
        this.btnSendFileEl.addEventListener("click",e=>{
            //simulate a click on the inputFileEl
            this.inputFileEl.click();
        });

        //why change not click?
        this.inputFileEl.addEventListener("change",e=>{
            this.uploadTask(e.target.files).then(resp=>{
                console.log(resp);
                resp.forEach(res=>{
                    this.getFBRef().push().set(res.fon['input-file']);
                })
        ...
        [...files].forEach(file=>{
            ...
            promises.push(new Promise((resolve,reject)=>{
                ...
                ajax.onload = e=>{
                    ...
                    try{
                    //responseText will return a json com o arquivo enviado;                    
    ...
    readFilesFB(){
        this.getFBRef().on('value', snapshot=>{
